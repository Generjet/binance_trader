...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///./crypto_turso_data.db' # changed from mysql
...
def create_database_if_not_exists():
    ...
    conn = sqlite3.connect('./crypto_turso_data.db') # Changed connection method
    cursor = conn.cursor()
    # SQLite does not need explicit database creation like MySQL
    # Tables will be created automatically if they don't exist when db.create_all() is called
    conn.close()
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == 'table':
        ...
        print("Run with 'python save2turso.py table' to update database tables.")
