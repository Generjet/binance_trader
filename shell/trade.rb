...
if order 
    ...
    if order.buy_amount != nil && order.sell_amount != nil
        u_can_buy = true
        u_can_sell = false
    elsif order.buy_amount != nil && order.sell_amount.nil? # && order.sell_price > (order.buy_price + 10.0)
        u_can_buy = false
        u_can_sell = true
    end
else
...
#     sell(order,now,lastPrice, lastSignal)
#     puts "UNFINISHED TRADE EXISTS: BUY=yes, SELL=nil" + " ==> U can SELL"
# else
#     buy(now,lastPrice, lastSignal, buy_amount, period) # Хэрэв buy=done, sell=done бол шинээр АВАХ захиалга өгч болно.
#     puts "PREVIOUS TRADE FINISHED: you can create new one" + " ==> U can buy"
# end
