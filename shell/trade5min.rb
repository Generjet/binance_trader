...
response = client.new_order(symbol: 'ETHUSDT', side: 'BUY', quantity: 0.0072, type: 'MARKET', timeInForce: 'GTC')

abort "=== BINANCE ==== BUY done !"
...
buy_amount = balance/possible_trades

puts " ***** POSSIBLE TRADES => #{possible_trades} and BALANCE = #{balance} and BUY_AMOUNT => #{buy_amount}"
# 2. ================== Read Signals ======================
# === from SQL =====
...
def buy(now,lastPrice, lastSignal, buy_amount, period)
    # response = client.new_orde r(symbol: 'ETHUSDT', side: 'BUY', price: 1945, quantity: 0.0294, type: 'LIMIT', timeInForce: 'GTC')
    order = Orders.new
    order.buy_date = now
